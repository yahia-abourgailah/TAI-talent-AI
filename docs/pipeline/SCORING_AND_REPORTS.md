...
Creating an application queues a `score_application` job in the same transaction (migration 0008), whatever code created it. The `worker` service (`python -m jobs work --loop`) picks it up within about a second.
...
On the test server use `--token-env TALENT_API_TOKEN` with a TA lead's token. Any invalid line refuses the whole file, by line number. A second run skips openings already there. `POST /v1/requisitions` accepts an optional `criteria_version` from a TA lead or an admin (the version in force is used when absent).
