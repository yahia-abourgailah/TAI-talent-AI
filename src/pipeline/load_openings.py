...
def _existing_open(api: Api) -> set[tuple[str, str, str, int, str, str, str]]:
    """Every open requisition the loading account can see, walking the cursor pages."""
    keys = set()
    cursor: str | None = None
    while True:
        query = f"/v1/requisitions?status=open&limit={_PAGE}"
        status, page = api.get(query if cursor is None else f"{query}&cursor={cursor}")
        if status != 200:
            raise LoadRefused([f"could not list existing requisitions (HTTP {status})"])
        for o in page["items"]:
            keys.add(
                (
                    o["brand"],
...
def load_openings(api: Api, lines: Sequence[OpeningLine]) -> dict[str, Any]:
    ...
    for line in lines:
        if line.key() in existing:
            already += 1
            continue
        status, body = api.post("/v1/requisitions", line.body())
        if status == 201:
            created.append({"line": line.line, "requisition_id": body["id"]})
