...
from candidates import queue, withdrawal
...
def test_a_locked_candidates_items_leave_the_queue(conn, make_candidate, use_proposed_list):
    """A candidate who asked us to stop keeping their data is gone from here too (BR-504)."""
    use_proposed_list(conn)
    _sign(conn, "band", 2)
    application = _apply(conn, make_candidate)
    _score(conn, application["id"])
    ...
    waiting = queue.list_open(conn, TA_LEAD, limit=200)
    assert candidate_id in {item["candidate_id"] for item in waiting}
    before = queue.summary(conn, TA_LEAD)["open"]

    withdrawal.record(conn, TA_LEAD, candidate_id, asked_how="phone")

    after = queue.list_open(conn, TA_LEAD, limit=200)
    assert candidate_id not in {item["candidate_id"] for item in after}
    assert queue.summary(conn, TA_LEAD)["open"] == before - 1
