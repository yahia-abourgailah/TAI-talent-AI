...
def _visible(client, account: str, brand: str) -> list[tuple[str, str]]:
    page = client.get(
        "/v1/requisitions", params={"brand": brand, "limit": 200}, headers=sign_in(client, account)
    ).json()
    return sorted((o["department"], o["owner_id"]) for o in page["items"])
...
def test_the_opening_keeps_the_criteria_version_from_the_file(api_client, tmp_path, brand):
    ...
    page = client.get(
        "/v1/requisitions", params={"brand": brand}, headers=sign_in(client, "ta-lead")
    ).json()
    (opening,) = page["items"]
    assert opening["criteria_version"] == "2026-08-04"
